...
export function WeeklySchedule({ isMasked, onEventClick, applications, growthLogs }: WeeklyScheduleProps) {
  ...
  const jobEventsCount = jobEvents.length
  const growthEventsCount = growthEvents.length
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 md:grid md:grid-cols-7 md:overflow-x-visible md:pb-0">
        {weekDays.map((day) => {
          ...
          return (
            <div
              key={dateISO}
              className={`min-w-[140px] flex-shrink-0 rounded-[14px] border border-[#E5E7EB] bg-white px-3 py-3 shadow-sm md:min-w-0 md:flex-shrink flex flex-col ${
            ...
            >
              <div className="mb-2 border-b border-[#E5E7EB] pb-2 flex items-center justify-between flex-shrink-0">
                <span className="text-xs font-medium text-[#555]">{formatDate(day)}</span>
                {eventCount > 0 && (
      ...
      <div className="mt-3 text-center text-xs text-[#6B7280]">
        今週の転職活動：{jobEventsCount}件　成長ログ：{growthEventsCount}件
      </div>
    </div>
  )
}
